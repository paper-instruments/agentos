...
impl JavascriptBenchmarkReport {
    ...
    pub fn render_markdown_with_comparison(
        ...
        let _ = writeln!(
            &mut markdown,
            "- Reproduce: `cargo run -p agentos-execution --bin node-import-bench -- --iterations {} --warmup-iterations {}`",
            self.config.iterations, self.config.warmup_iterations
        );
        let _ = writeln!(&mut markdown);
    ...
    fn json_artifact<'a>(
        ...
        JavascriptBenchmarkArtifact {
            ...
            command: format!(
                "cargo run -p agentos-execution --bin node-import-bench -- --iterations {} --warmup-iterations {}",
                self.config.iterations, self.config.warmup_iterations
            ),
            config: &self.config,
            host: &self.host,
