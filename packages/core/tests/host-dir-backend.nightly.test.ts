	...
	test.runIf(process.platform === "win32")(
		...
		async () => {
			...
			const source = [
				...
				'print(f\'agentos-doc-preflight={os.getcwd()}|{oct(os.stat(".").st_mode)}|{os.access(".", os.W_OK)}\', flush=True)',
				"with open('preflight-text.tmp', 'w') as probe: probe.write('text')",
				"with open('preflight-binary.tmp', 'w+b') as probe: probe.write(b'binary')",
				"os.unlink('preflight-text.tmp'); os.unlink('preflight-binary.tmp')",
